...
function RegisterPage() {
  ...
  const registerSchema = z.object({
    ...
    confirm_password: z.string().min(6, t('auth.validation.password')),
  ...
  } = useForm<RegisterPayload & { confirm_password: string }>({
    resolver: zodResolver(registerSchema),
  })

  const onSubmit = async (values: RegisterPayload & { confirm_password: string }) => {
    try {
      setErrorMessage('')
      ...
      <div className="flex w-full items-center justify-center bg-[var(--color-bg)] px-6 lg:w-1/2">
        <div className="w-full max-w-md">
          ...
          <div className="mb-10">
            ...
            <h1 className="text-3xl font-extrabold text-[var(--color-text)]">{t('auth.create_account')}</h1>
            <p className="mt-2 text-[var(--color-text-muted)]">{t('auth.register_subtitle')}</p>
          </div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            <div>
              ...
              />
              {errors.full_name && <p className="mt-1.5 text-sm text-[var(--color-danger)]">{errors.full_name.message}</p>}
            </div>
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold text-[var(--color-text)]">{t('auth.confirm_password')}</label>
              <div className="relative">
                <input
              ...
              {errors.confirm_password && <p className="mt-1.5 text-sm text-[var(--color-danger)]">{errors.confirm_password.message}</p>}
            </div>
          ...
          <p className="mt-8 text-center text-sm text-[var(--color-text-muted)]">
            {t('auth.have_account')}{' '}
            <Link to="/login" className="font-semibold text-[var(--color-primary)] transition-colors hover:text-[var(--color-primary-soft)]">
              {t('auth.login_here')}
            </Link>
          </p>
        </div>
